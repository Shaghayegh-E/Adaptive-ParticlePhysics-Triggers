...
#
#    return Sas01_tot1, Sas04_tot1, Sht_tot1, S_npvs1, Sas01_tot2,  Sas04_tot2, Sht_tot2, S_npvs2, Bas01_tot, Bas04_tot, Bht_tot, B_npvs #,data_ht, data_score, data_npv
path = "new_Data/Trigger_food.h5" 
#Matched_data_2016_with04.h5"
...
# --- Save & show ---
plt.savefig('paper/bht_rate_pidMC.pdf', bbox_inches='tight')
plt.show()
plt.close()
...
plt.savefig('paper/bas_rate_pidMC.pdf', bbox_inches='tight')
plt.show()
plt.close()
...
# Save & show
plt.savefig("paper/sht_rate_pidMC.pdf", bbox_inches="tight")
plt.show()
plt.close()
...
plt.ylim(0.5,2.50)
# --- Save and show ---
plt.savefig("paper/L_sht_rate_pidMC.pdf", bbox_inches="tight")
plt.show()
plt.close()
...
# --- define initial efficiencies for all AD signal samples ---
eff0_ttbar1 = E3_1[0]
eff0_ttbar4 = E3_4[0]
eff0_AA1 = E5_1[0]
eff0_AA4 = E5_4[0]
...
# Example plot usage:
plt.plot(time, E3_1/eff0_ttbar1, label=fr'ttbar (dim=1, $\epsilon[t_0]$={eff0_ttbar1:.2f})',
...
# --- Save and show ---
plt.savefig("paper/sas_rate_pidMC2.png", bbox_inches="tight")
plt.show()
plt.close()
...
# =========================================================
# === 1. HT Trigger: Background Rate (PD Controller) ======
# =========================================================
time = np.linspace(0, 1, len(R1))
plt.figure(figsize=(10, 6))
plt.plot(time, R1, label='Constant Menu', color='tab:blue', linewidth=3, linestyle='dashed')
...
plt.savefig('paper/bht_rate_pidMC.png', bbox_inches='tight', dpi=300)
plt.close()

# =========================================================
# === 2. AD Trigger: Background Rate (PD Controller) ======
# =========================================================
time = np.linspace(0, 1, len(E1_1))
plt.figure(figsize=(10, 6))
plt.plot(time, E1_1, label='Constant Menu, model dim=1', color='mediumblue', linewidth=3, linestyle='dotted')
plt.plot(time, E1_4, label='Constant Menu, model dim=4', color='cyan', linewidth=3, linestyle='dashed')
plt.plot(time, E2_1, label='PD Controller, model dim=1', color='mediumblue', linewidth=2.5, linestyle='solid')
plt.plot(time, E2_4, label='PD Controller, model dim=4', color='cyan', linewidth=2.5, linestyle='solid')
plt.axhline(y=0.28*400, color='gray', linestyle='--', linewidth=1.5)
plt.axhline(y=0.22*400, color='gray', linestyle='--', linewidth=1.5)
...
plt.savefig('paper/bas_rate_pidMC.png', bbox_inches='tight', dpi=300)
plt.close()

# =========================================================
# === 3. HT Trigger: Cumulative Signal Efficiency =========
# =========================================================
time = np.linspace(0, 1, len(R3))
eff0_ttbar = R3[0]
eff0_AA = R5[0]
plt.figure(figsize=(10, 6))
plt.plot(time, R3/eff0_ttbar, label=fr'ttbar ($\epsilon[t_0]$ = {eff0_ttbar:.2f})',
         color='goldenrod', linewidth=2.5, linestyle='dashed')
plt.plot(time, R5/eff0_AA, label=fr'HToAATo4B ($\epsilon[t_0]$ = {eff0_AA:.2f})',
...
plt.legend(title='HT Constant Menu', frameon=True)
plt.ylim(0.5, 1.1)
plt.savefig('paper/sht_rate_fixed2.pdf', bbox_inches='tight', dpi=300)
plt.close()

# =========================================================
# === 4. HT Trigger: Local Signal Efficiency ==============
# =========================================================
time = np.linspace(0, 1, len(L_R3))
eff0_ttbar = L_R3[0]
eff0_AA = L_R5[0]
plt.figure(figsize=(10, 6))
plt.plot(time, L_R3/eff0_ttbar, label=fr'ttbar ($\epsilon[t_0]$ = {eff0_ttbar:.2f})',
         color='goldenrod', linewidth=2.5, linestyle='dashed')
plt.plot(time, L_R5/eff0_AA, label=fr'HToAATo4B ($\epsilon[t_0]$ = {eff0_AA:.2f})',
         color='seagreen', linewidth=2.5, linestyle='dashed')
plt.ylim(0, 1.2)
plt.xlabel('Time (Fraction of Run)', loc='center')
plt.ylabel('Relative Efficiency', loc='center')
plt.legend(title='HT Constant Menu', frameon=True)
plt.grid(True, linestyle='--', alpha=0.6)
plt.savefig('paper/L_sht_rate_fixed2.pdf', bbox_inches='tight', dpi=300)
plt.close()

# =========================================================
# === 5. HT Trigger: Cumulative Signal Efficiency (PD) ====
# =========================================================
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.lines as mlines
...
# =========================================================
# === HT Trigger: Cumulative Signal Efficiency (Const + PD)
# =========================================================
...
# --- Define time axis and initial efficiencies ---
...
# --- Save & show ---
plt.savefig("paper/sht_rate_pidMC2.pdf", bbox_inches="tight", dpi=300)
plt.show()
plt.close()


# =========================================================
# === 6. HT Trigger: Local Signal Efficiency (PD) =========
# =========================================================
import numpy as np
import matplotlib.pyplot as plt
import matplotlib.lines as mlines
...
# =========================================================
# === HT Trigger: Local Signal Efficiency (Constant + PD) ==
# =========================================================

# --- Define consistent styles and colors ---
styles = {
    "Constant": {"linestyle": "dashed", "linewidth": 2.5},
...
# --- Constant Menu ---
plt.plot(time, np.array(L_R3[:n]) / eff0_ttbar_const,
         label=fr"Constant Menu, ttbar ($\epsilon[t_0]={eff0_ttbar_const:.2f}$)",
         color=colors["ttbar"], **styles["Constant"])

plt.plot(time, np.array(L_R5[:n]) / eff0_higgs_const,
         label=fr"Constant Menu, HToAATo4B ($\epsilon[t_0]={eff0_higgs_const:.2f}$)",
         color=colors["HToAATo4B"], **styles["Constant"])

# --- PD Controller ---
plt.plot(time, np.array(L_R4[:n]) / eff0_ttbar_pd,
         label=fr"PD Controller, ttbar ($\epsilon[t_0]={eff0_ttbar_pd:.2f}$)",
         color=colors["ttbar"], **styles["PD"])

plt.plot(time, np.array(L_R6[:n]) / eff0_higgs_pd,
         label=fr"PD Controller, HToAATo4B ($\epsilon[t_0]={eff0_higgs_pd:.2f}$)",
         color=colors["HToAATo4B"], **styles["PD"])
...
# --- Legend setup (2 columns: Constant | PD) ---
header_const = mlines.Line2D([], [], color="none", linestyle="none")
header_pd    = mlines.Line2D([], [], color="none", linestyle="none")

# Dummy handles (matching plotted lines)
const_ttbar = mlines.Line2D([], [], color=colors["ttbar"],     **styles["Constant"])
const_higgs = mlines.Line2D([], [], color=colors["HToAATo4B"], **styles["Constant"])
pd_ttbar    = mlines.Line2D([], [], color=colors["ttbar"],     **styles["PD"])
pd_higgs    = mlines.Line2D([], [], color=colors["HToAATo4B"], **styles["PD"])

# Order for ncol=2 layout
...
# --- Save and show ---
plt.savefig("paper/L_sht_rate_pidMC2.pdf", bbox_inches="tight", dpi=300)
plt.show()
plt.close()

# =========================================================
# === 7. AD Trigger: Cumulative Signal Efficiency =========
# =========================================================
...
eff0_AA4 = E5_4[0]
plt.figure(figsize=(10, 6))
plt.plot(time, np.array(E3_1[:n])/eff0_ttbar1, label=fr'ttbar (dim=1, $\epsilon[t_0]$={eff0_ttbar1:.2f})', color='goldenrod', linewidth=2.5, linestyle='dashed')
plt.plot(time, np.array(E3_4[:n])/eff0_ttbar4, label=fr'ttbar (dim=4, $\epsilon[t_0]$={eff0_ttbar4:.2f})', color='orangered', linewidth=2.5, linestyle='dashed')
plt.plot(time, np.array(E5_1[:n])/eff0_AA1, label=fr'HToAATo4B (dim=1, $\epsilon[t_0]$={eff0_AA1:.2f})', color='limegreen', linewidth=2.5, linestyle='dashed')
plt.plot(time, np.array(E5_4[:n])/eff0_AA4, label=fr'HToAATo4B (dim=4, $\epsilon[t_0]$={eff0_AA4:.2f})', color='seagreen', linewidth=2.5, linestyle='dashed')
plt.xlabel('Time (Fraction of Run)',loc='center')
plt.ylabel('Relative Cumulative Efficiency',loc='center')
plt.ylim(0.5, 2.5)
plt.grid(True, linestyle='--', alpha=0.6)
plt.legend(title='AD Constant Menu', frameon=True)
plt.savefig('paper/sas_rate_fixed2.pdf', bbox_inches='tight', dpi=300)
plt.close()

# =========================================================
# === 8. AD Trigger: Local Signal Efficiency ==============
# =========================================================
...
plt.savefig('paper/L_sas_rate_fixed2.pdf', bbox_inches='tight', dpi=300)
plt.close()
...
# =========================================================
# === AD Trigger: Cumulative Signal Efficiency (Const + PD)
# =========================================================
...
# --- Define time axis ---
n = min(len(E3_1), len(E4_1), len(E3_4), len(E4_4),
        len(E5_1), len(E6_1), len(E5_4), len(E6_4))
time = np.linspace(0, 1, n)
...
# --- Legend setup with per-column headers ---
header_const = mlines.Line2D([], [], color="none", linestyle="none")
header_pd    = mlines.Line2D([], [], color="none", linestyle="none")
...
# --- Save and show ---
plt.savefig("paper/sas_rate_pidMC2.pdf", bbox_inches="tight", dpi=300)
plt.show()
plt.close()
...
# =========================================================
# === AD Trigger: Local Signal Efficiency (Const + PD)
# =========================================================

# --- Define time axis ---
n = min(len(L_E3_1), len(L_E4_1), len(L_E3_4), len(L_E4_4),
        len(L_E5_1), len(L_E6_1), len(L_E5_4), len(L_E6_4))
time = np.linspace(0, 1, n)
...
# --- Constant Menu ---
plt.plot(time, np.array(L_E3_1[:n]) / eff0_ttbar1_const,
         label=fr"Constant Menu, ttbar (dim=1, $\epsilon[t_0]={eff0_ttbar1_const:.2f}$)",
         color=colors["ttbar_dim1"], **styles["Constant"])
plt.plot(time, np.array(L_E3_4[:n]) / eff0_ttbar4_const,
         label=fr"Constant Menu, ttbar (dim=4, $\epsilon[t_0]={eff0_ttbar4_const:.2f}$)",
         color=colors["ttbar_dim4"], **styles["Constant"])
plt.plot(time, np.array(L_E5_1[:n]) / eff0_higgs1_const,
         label=fr"Constant Menu, HToAATo4B (dim=1, $\epsilon[t_0]={eff0_higgs1_const:.2f}$)",
         color=colors["higgs_dim1"], **styles["Constant"])
plt.plot(time, np.array(L_E5_4[:n]) / eff0_higgs4_const,
         label=fr"Constant Menu, HToAATo4B (dim=4, $\epsilon[t_0]={eff0_higgs4_const:.2f}$)",
         color=colors["higgs_dim4"], **styles["Constant"])

# --- PD Controller ---
plt.plot(time, np.array(L_E4_1[:n]) / eff0_ttbar1_pd,
         label=fr"PD Controller, ttbar (dim=1, $\epsilon[t_0]={eff0_ttbar1_pd:.2f}$)",
         color=colors["ttbar_dim1"], **styles["PD"])
plt.plot(time, np.array(L_E4_4[:n]) / eff0_ttbar4_pd,
         label=fr"PD Controller, ttbar (dim=4, $\epsilon[t_0]={eff0_ttbar4_pd:.2f}$)",
         color=colors["ttbar_dim4"], **styles["PD"])
plt.plot(time, np.array(L_E6_1[:n]) / eff0_higgs1_pd,
         label=fr"PD Controller, HToAATo4B (dim=1, $\epsilon[t_0]={eff0_higgs1_pd:.2f}$)",
         color=colors["higgs_dim1"], **styles["PD"])
plt.plot(time, np.array(L_E6_4[:n]) / eff0_higgs4_pd,
...
handles = [
    header_const, const_ttbar_dim1, const_ttbar_dim4, const_higgs_dim1, const_higgs_dim4,
    header_pd, pd_ttbar_dim1, pd_ttbar_dim4, pd_higgs_dim1, pd_higgs_dim4
]
labels = [
    "Constant Menu",
...
plt.ylim(0.5,4.3)
# --- Save and show ---
plt.savefig("paper/L_sas_rate_pidMC2.pdf", bbox_inches="tight", dpi=300)
plt.show()
plt.close()
